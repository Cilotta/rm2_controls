...
#include "rm2_referee/ui/time_change_ui.h"
...
void TimeChangeUi::update()
{
  ...
  UiBase::display(rclcpp::Time::now());
}
...
void TimeChangeGroupUi::update()
{
  ...
  for (auto character : character_vector_)
    character.second->setOperation(rm2_referee::GraphOperation::UPDATE);
...
void TimeChangeUi::updateForQueue()
{
  ...
  if (graph_queue_ && character_queue_ && rclcpp::Time::now() - last_send_ > delay_)
  {
...
void CapacitorTimeChangeUi::updateRemainCharge(const double remain_charge, const rclcpp::Time& time)
{
...
void EffortTimeChangeUi::updateJointStateData(const sensor_msgs::JointState::ConstPtr data, const rclcpp::Time& time)
{
  int max_index = 0;
  if (!data->name.empty())
  {
...
void ProgressTimeChangeUi::updateEngineerUiData(const rm2_msgs::EngineerUi::ConstPtr data,
...
void DartStatusTimeChangeUi::updateDartClientCmd(const rm2_msgs::DartClientCmd::ConstPtr data,
                                                 const rclcpp::Time& last_get_data_time)
{
...
void RotationTimeChangeUi::updateConfig()
{
  if (chassis_mode_ != rm2_msgs::ChassisCmd::RAW)
    ...
    graph_->setColor(rm2_referee::GraphColor::GREEN);
  if (!tf_buffer_.canTransform(gimbal_reference_frame_, chassis_reference_frame_, rclcpp::Time(0)))
    return;
  try
  {
    int angle;
    ...
    quatToRPY(
        tf_buffer_.lookupTransform(chassis_reference_frame_, gimbal_reference_frame_, rclcpp::Time(0)).transform.rotation,
        roll, pitch, yaw);
...
void RotationTimeChangeUi::updateChassisCmdData(const rm2_msgs::ChassisCmd::ConstPtr data)
{
...
void LaneLineTimeChangeGroupUi::updateJointStateData(const sensor_msgs::JointState::ConstPtr data, const rclcpp::Time& time)
{
  if (!tf_buffer_.canTransform("yaw", reference_frame_, rclcpp::Time(0)))
    return;
  try
  {
    double roll, pitch, yaw;
    quatToRPY(tf_buffer_.lookupTransform("yaw", reference_frame_, rclcpp::Time(0)).transform.rotation, roll, pitch, yaw);
    pitch_angle_ = pitch;
  }
  catch (tf2::TransformException& ex)
  {
...
void BalancePitchTimeChangeGroupUi::calculatePointPosition(const rm2_msgs::BalanceStateConstPtr& data,
...
void PitchAngleTimeChangeUi::updateJointStateData(const sensor_msgs::JointState::ConstPtr data, const rclcpp::Time& time)
{
  for (unsigned int i = 0; i < data->name.size(); i++)
...
void JointPositionTimeChangeUi::updateConfig()
{
  ...
  if (abs(proportion) > 0.96)
    graph_->setColor(rm2_referee::GraphColor::BLACK);
  else if (abs(proportion) > 0.8)
    graph_->setColor(rm2_referee::GraphColor::PINK);
  else if (abs(proportion) > 0.6)
    graph_->setColor(rm2_referee::GraphColor::PURPLE);
...
void JointPositionTimeChangeUi::updateJointStateData(const sensor_msgs::JointState::ConstPtr data, const rclcpp::Time& time)
{
  for (unsigned int i = 0; i < data->name.size(); i++)
...
void BulletTimeChangeUi::updateBulletData(const rm2_msgs::BulletAllowance& data, const rclcpp::Time& time)
{
...
void TargetDistanceTimeChangeUi::updateTargetDistanceData(const rm2_msgs::TrackData::ConstPtr& data)
{
  if (data->id == 0)
    return;
  ...
  try
  {
    geometry_msgs::TransformStamped transform_stamped = tf_buffer_.lookupTransform("base_link", "odom", rclcpp::Time(0));
...
void FriendBulletsTimeChangeGroupUi::updateBulletsData(const rm2_referee::BulletNumData& data)
{
  if (data.header_data.sender_id == rm2_referee::RobotId::RED_HERO ||
      data.header_data.sender_id == rm2_referee::RobotId::BLUE_HERO)
    hero_bullets_ = data.bullet_42_mm_num;
  else if (data.header_data.sender_id == rm2_referee::RobotId::RED_STANDARD_3 ||
           data.header_data.sender_id == rm2_referee::RobotId::BLUE_STANDARD_3)
    standard3_bullets_ = data.bullet_17_mm_num;
  else if (data.header_data.sender_id == rm2_referee::RobotId::RED_STANDARD_4 ||
           data.header_data.sender_id == rm2_referee::RobotId::BLUE_STANDARD_4)
    standard4_bullets_ = data.bullet_17_mm_num;
  else if (data.header_data.sender_id == rm2_referee::RobotId::RED_STANDARD_5 ||
           data.header_data.sender_id == rm2_referee::RobotId::BLUE_STANDARD_5)
